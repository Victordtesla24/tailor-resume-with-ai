...
from src.models import AIModelManager, AnthropicModel, OpenAIModel
...
def test_openai_model_processing(mock_openai, mock_config):
    ...
    # Setup mock response
    mock_response = Mock()
    mock_response.choices = [Mock(message=Mock(content="Tailored resume content"))]
    mock_openai.return_value.chat.completions.create.return_value = mock_response

    model = OpenAIModel(mock_config.get("OPENAI_API_KEY"))
    result = model.process_resume("Original resume", "Job description", ["summary", "experience"])
...
def test_model_error_handling(model_manager):
    ...
    with pytest.raises(RuntimeError):
        model_manager.process_resume("Original resume", "Job description", ["summary"])
